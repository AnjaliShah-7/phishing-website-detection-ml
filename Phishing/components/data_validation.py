...
from Phishing.constant.training_pipeline import SCHEMA_FILE_PATH,TARGET_COLUMN
from scipy.stats import ks_2samp
import pandas as pd
import os,sys
from Phishing.utils.main_utils.utils import read_yaml_file,write_yaml_file
...
class DataValidation:
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            if not status:
              raise Exception("Train dataframe contains non-numerical columns.")

            status = self.is_numerical_column_exist(test_dataframe)
            if not status:
              raise Exception("Test dataframe contains non-numerical columns.")
